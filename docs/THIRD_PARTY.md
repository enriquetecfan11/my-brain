...
Release binaries built with `--with-ui` embed the compiled `cbm-dashboard/ui/`
frontend bundle. Its npm dependencies (React, react-force-graph-2d, Vite,
and friends) are all under permissive
licenses (MIT / ISC / Apache-2.0 / Zlib); the exact set is recorded in
`cbm-dashboard/ui/package.json` and `cbm-dashboard/ui/package-lock.json`, and the per-package
